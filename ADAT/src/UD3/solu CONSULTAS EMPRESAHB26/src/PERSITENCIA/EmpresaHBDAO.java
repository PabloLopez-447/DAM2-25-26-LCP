...
//            // 2. Buscar departamento
            ...
            /*
     Este es el ?nico cambio que Hibernate necesita para actualizar la base de datos, porque:
     El lado MANY (Empregado) contiene la clave for?nea.Hibernate solo mira ese lado para generar el UPDATE
     Por tanto: Con esta l?nea, la relaci?n queda correctamente persistida en la base de datos.       
             */
            empregado.setDepartamento(departamento);          // Lado "muchos" ? establecemos el departamento del empleado

            /*  Esta l?nea NO es necesaria para la base de datos, 
     pero S? es necesaria para mantener la coherencia en memoria.
     Si no la pones:
    empregado.getDepartamento() ? mostrar? el nuevo departamento
    pero novoDepartamento.getEmpregados() ? NO incluir? al empleado
             */
...
Al pasar la Session como argumento, evitas el coste de abrir y cerrar conexiones constantemente. 
     ...
     *
     * @param listaEntrada
     * @return Lista de objetos Habilidade que han sido insertados con ?xito.
     */
    public static List<Habilidade> insertarHabilidadesDAO(List<Habilidade> listaEntrada) throws HibernateException {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            for (Habilidade hab : listaEntrada) {
                // Comprobamos si ya existe en la BD usando la sesi?n actual
                Habilidade existente = buscarHabilidadePorNome(hab.getNome(), session);

                if (existente == null) {
                    session.save(hab); // SQL Server genera el ID aqu?
                    insertadas.add(hab);
                }
            }
...
// 2. Quitamos la habilidad de cada empleado (esto limpia la tabla intermedia)
            ...
            session.delete(h); // borra tam?n EMPREGADO_HABILIDADE
            tx.commit();
            return 0;
     ...
     * @param nss
     * @param habilidades
     * @return -1 ? empleado no existe 0 ? operaci?n correcta
     */
    ...
    public static int gardarOuActualizarVehiculoDAO(String nss, Vehiculo v) throws HibernateException {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (existeMat != null && !existeMat.getNss().equals(nss)) {
                return -2; // matr?cula xa usada por outro empregado
            }
            ...
            if (vehEmp == null) {
                ...
                //el lado propietario (Vehiculo), porque es el ?nico que escribe la relaci?n en la base de datos.
                //El otro lado es solo de navegaci?n.
                v.setEmpregado(emp);
                v.setNss(emp.getNss());   // PK = FK
                //emp.setVehiculo(vehEmp);  //no hace falta  ponerlo funciona igual. Pero si luego se utiliza si que har?a falta para que tenga constancia
                session.save(v);
            } else {
                // Actualizaci?n           
    ...
    public static int borrarVehiculoDAO(String nss) throws HibernateException {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            Empregado emp = buscarEmpregadoPorNSS(nss, session);
            if (emp == null) {
                return -2; // NSS non v?lido ? non existe empregado
            }

            // 2. Recuperar o veh?culo polo NSS (PK = FK)
            Vehiculo v = session.get(Vehiculo.class, nss);
            if (v == null) {
                return -1; // o empregado existe pero non ten veh?culo
            }

            // 3. Romper a relaci?n no lado inverso (Empregado)
            /* Se borras o veh?culo sen limpar o empregado:
    ...
    public static List<EmpregadoProxectoDTO> listarEmpleadosProxectoDAO() {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            return session.createQuery(hql, EmpregadoProxectoDTO.class).list(); // getResultList() es JPA e list() m?is com?n en Hibernate puro
        }
    }
    ...
    public static boolean borrarProxectoPorNome(String nomeProxecto) {
        ...
        ao borrar un proxecto Hibernate non eliminar?a automaticamente as relaci?ns en EMPREGADO_PROXECTO 
    ...
    public static String quitarSupervisorDAO(String nss) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            /// Caso 3: empregado ten supervisor ? constru?mos mensaxe correcta 
...
//    }
     ...
     *
     * Devolve: 0 ? horas extras insertadas (non exist?a esa data) 1 ? horas
     * extras actualizadas (xa exist?an) -1 ? non existe un empregado con ese
     * NSS -2 ? o empregado existe pero non ? fixo -3 ? erro na operaci?n
     * (excepci?n)
     *
     * @param nss
     * @param fecha
    ...
    public static int insertarModificarHorasExtras(String nss, LocalDate fecha, double horas) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            //Para saber si es una insercci?n o actualizaci?n 
            boolean existe = mapa.containsKey(fecha);
            mapa.put(fecha, horas); // inserta o actualiza autom?ticamente
            session.update(fijo);
            tx.commit();
    ...
    public static double totalHorasExtrasMesDAO(String nss, int mes, int ano) {
        // Usamos try-with-resources para asegurar el cierre autom?tico de la sesi?n
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            // 2. Consulta HQL sobre la colecci?n Map 'horasextras'
        ...
        """;
            ...
            // Al no usar transacci?n manual, el catch captura el error sin conflictos
            System.err.println("Erro t?cnico: " + e.getMessage());
            return -3.0;
        }
    }
    ...
    public static List<String> obtenerNombresLugaresDAO(String nomeDepto) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            // 2. Si existe, usamos su m?todo getter
            List<String> nombres = new ArrayList<>();
            for (Lugar l : depto.getLugares()) {
                nombres.add(l.getLugar());
            }
            //con consulta hql ser?a
            /*
    ...
    public static int insertarDepartamentoCompleto(Departamento dpto, Set<String> lugares,
     ...
     // 1. Comprobar se o director xa dirixe outro departamento 
            ...
            if (existente1 != null) {
                return -3; // o director xa ? director noutro departamento 

            }
            tx = session.beginTransaction();
            /*
    O departamento ? o propietario l?xico da relaci?n co director, pero en Hibernate sempre se debe actualizar o lado que cont?n
    a clave for?nea (FK). Neste caso, a FK NSSDirector est? na t?boa DEPARTAMENTO, polo que debemos asignar
    o director chamando a  dpto.setDirector(director).
    Non ? necesario facer nada no outro sentido (deptodirector.setDepartamento),
    porque Empregadofixo NON ten unha FK nin mapea esta relaci?n.
             */
            ...
            //  Al ser Set en el POJO, addAll filtra duplicados autom?ticamente
            // es componente 
            if (funciones != null) {
                dpto.getFunciones().addAll(funciones);
            }

            // Los lugares se a?aden al Set de la entidad
            if (lugares != null) {
                for (String l : lugares) {
                    Lugar novoLugar = new Lugar(l);
                    novoLugar.setDepartamento(dpto); //?nico lado que Hibernate usa para actualizar a base de datos ? o lado propietario, ? dicir:? o lado que cont?n a FK Num_departamento.
                    dpto.getLugares().add(novoLugar); // NON ? necesario para insertar en la bd BD, pero SI para manter o modelo en memoria consistente.
                    //Se non o fas, dpto.getLugares() non mostrar? o novo lugar ata recargar desde a BD.
                }
            }
