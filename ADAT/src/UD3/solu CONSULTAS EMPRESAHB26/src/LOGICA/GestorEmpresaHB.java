...
public class GestorEmpresaHB {
  ...
  NOTA   :  CAMBI� FECHADENACIMEINTO EN LOS POJOS Y EN EL FICHERO HBM.XML POR localdate
    ...
    public static void cambiarDepartamentoEmpregado(
        ...
        switch (resultado) {
            case 0 -> System.out.println("Departamento do empregado actualizado correctamente.");
            case -1 -> System.out.println("ERROR: O empregado non existe.");
            case -2 -> System.out.println("ERROR: O departamento non existe.");
            case -3 -> System.out.println("O empregado xa pertence a ese departamento.");
            default -> System.out.println("Erro inesperado ao cambiar o departamento.");
        }
    }

    public static void altaHabilidades(List<Habilidade> listaEntrada) {
        try {
            List<Habilidade> insertadas = EmpresaHBDAO.insertarHabilidadesDAO(listaEntrada);
            if (insertadas.isEmpty()) {
                System.out.println("Non se insertou ningunha habilidade nova.");
            } else {
                System.out.println("Habilidades insertadas correctamente:");
                for (Habilidade h : insertadas) {
                    System.out.println(" - " + h.getNome() + " (ID: " + h.getId() + ")");
                }
            }
        } catch (HibernateException e) {
            System.out.println("Erro ao insertar habilidades.");
        }
    }
