...
public class GestorEmpresaHBConsultas {
    ...
    System.out.printf("%-10s %-25s %-15s%n", "Número", "Nome", "Lugar");
 ...
 public static void visualizarEmpregadosOrdenados() {
    ...
    System.out.printf("%-10s %-30s %-15s %-15s %-15s%n",
            "NSS", "Nome Completo", "Departamento", "Tipo empregado", "num. teléfonos");
...
public static void visualizarListadoOrdenadoDTOLOGICA() {
    ...
    for (EmpregadoListadoDTO emp : lista) {
        // Como el departamento es obligatorio, lo mostramos siempre en mayúsculas para que resalte
        System.out.printf("%-10s %-30s %-20s %-15s %-15s\n",
                emp.getNss(),
                emp.getNomeCompleto(),
...
/*eXERCICO 7
D
    MÉTODO mostrarDirectores:  Recibe a lista de DTOs xerada polo DAO e imprime a información
    nun formato tabulado.
    Se o supervisor é NULL (porque o LEFT JOIN non atopou supervisor),
    móstrase "(sen supervisor)".
*/
public static void mostrarDirectores() {
    ...
    for (DirectorDepartamentoDTO dto : lista) {
        ...
        if (supervisor == null) supervisor = "(sen supervisor)";
...
//b
  public static void visualizarEmpregadosQueSuperanDirectorMax() {
    ...
    if (resultados.isEmpty()) {
        System.out.println("Non se atoparon empregados que gañen máis que o director que máis gaña.");
        return;
    }
  ...
  public static void visualizarVaronsSupervisoresPorRango(float limInf, float limSup) {
    ...
    if (resultados.isEmpty()) {
        System.out.printf("Non se atoparon varóns supervisores con salario entre %.2f e %.2f.%n", limInf, limSup);
        return;
    }

    System.out.println("\nLISTADO DE VARÓNS SUPERVISORES (Rango: " + limInf + " - " + limSup + ")");
  ...
  public static void subirSalarioPorProxectos(float porcentaxe) {
    ...
    if (actualizados > 0) {
        System.out.printf("Subida do %.1f%% aplicada con éxito a %d empregado(s).%n", porcentaxe, actualizados);
    } else if (actualizados == 0) {
        System.out.println("Non se atoparon empregados para actualizar.");
    } else {
        System.out.println("Houbo un erro na operación.");
    }
}
